...
describe("X12parser", function() {
  describe("#constructor()", function() {
    ...
    it("Should return an X12parser",
       function() { assert(myParser instanceof X12parser); });
    it("Should have a pipe function",
       function() { assert.strictEqual(typeof myParser.pipe, "function"); });
  ...
  describe("835 File Tests", function() {
    ...
    it("Should parse single line files", function() {
      const myParser = new X12parser();
      const testFile =
          createReadStream("./test/testFiles/835/profeeOneLine.edi");
      let counter = 0; // So ugly... This should be done nicer
      const {finished} = require("./testFiles/835/profee-done");
      testFile.pipe(myParser).on("data", (data) => {
        assert.deepStrictEqual(data, finished[counter]);
    ...
    it("Should parse multiple transactions (ISA) in a single file", function() {
      const myParser = new X12parser();
      const testFile =
          createReadStream("./test/testFiles/835/profeeMultiple.edi");
      let counter = 0; // So ugly... This should be done nicer
      const {finished} = require("./testFiles/835/profee-done");
      testFile.pipe(myParser).on("data", (data) => {
        if (!finished[counter])
    ...
    it("Should parse multiline files without delimiter (LF/CRLF is delimiter)",
       function() {
         const myParser = new X12parser();
         const testFile =
             createReadStream("./test/testFiles/835/multiLineNotDelimited.edi");
         let counter = 0; // So ugly... This should be done nicer
         const {finished} = require("./testFiles/835/profee-done");
         testFile.pipe(myParser).on("data", (data) => {
           assert.deepStrictEqual(data, finished[counter]);
