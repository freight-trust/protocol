...
describe("Group", function() {
  ...
  const schema = {
    version :
        "005010X221A1",   // What version of the transaction set this applies to
  ...
  describe("#add()", function() {
    it("Should group nested data", function() {
      const callDone =
          (group) => { assert.deepStrictEqual(group.data, grouped); };
      const myGroup = new Group(schema.groups[1], clp, callDone);
      svcArray.forEach(
          (svc) => { assert.strictEqual(myGroup.add(svc), true); });
      myGroup.terminate();
    });
    it("Should be able to group multiple nested groups", function() {
      const callDone =
          (group) => { assert.deepStrictEqual(group.data, groupedByISA); };
