...
if(Weight == item
Map[orderId].weight)
{ statsMap[orderId].checkPoint="Product is checked by Costoms"; // Updates
currentStatusOfOrder.
statsMap[orderId].timeTheEventCalled=now;
passed(orderId);}
else
{ statsMap[ord
erId].checkPoint="Product Weight is not correct"; // Updates
currentStatusOfOrder.
statsMap[orderId].timeTheEventCalled=now;
unpassed(orderId);
}
}
//Checks wheather manufacturer_quantity is Und
er IdealWeight or Over IdealWeight
function ApproveInCategory(uint orderId) public returns(string) {
if(itemMap[orderId].weight <= Idealweight)
{ statsMap[orderId].checkPoint="Product is Approved by Costoms"; // Updates
currentStatusOfOrder.
statsMap[orderId].timeTheEventCalled=now;
underweight(orderId);
}
else
{ statsMap[orderId].checkPoint="Product is Over Weight, Charges will be applied"; //
Updates currentStatusOfOrde
r.
statsMap[orderId].timeTheEventCalled=now;
overweight(orderId);
}
